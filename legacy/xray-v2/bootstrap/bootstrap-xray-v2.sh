...
#
# REPO_RAW_URL — required raw repository base URL.
#
...
prompt_vless_urls_if_needed() {
    ...
    if ! outbound_ready T; then
        ...
        printf '%s\n' '  vless://00000000-0000-0000-0000-000000000000@example.com:443?security=reality&sni=www.microsoft.com&fp=chrome&pbk=<PUBLIC_KEY>&sid=<SHORT_ID>&type=tcp&flow=xtls-rprx-vision#T' >/dev/tty
    ...
    if ! outbound_ready A; then
        ...
        printf '%s\n' 'Neutral example:' >/dev/tty
        printf '%s\n' '  vless://00000000-0000-0000-0000-000000000000@example.net:443?security=reality&sni=www.apple.com&fp=chrome&pbk=<PUBLIC_KEY>&sid=<SHORT_ID>&type=tcp&flow=xtls-rprx-vision#A' >/dev/tty
        if [ -n "$t_value" ]; then
...
[ -n "$REPO_RAW" ] || die "pass REPO_RAW_URL explicitly"
